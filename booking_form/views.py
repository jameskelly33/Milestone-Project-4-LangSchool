...
def booking_form(request, course):
    ...
    if request.method == 'POST':
        form_data = {
                ...
                'total_course_cost': int(course.cost_per_week) * int(
                                    request.POST['course_length']),
        ...
        if int(form_data.get('age')) < course.minimum_age:
            messages.error(request, f'Sorry, the minimum age for this\
                 course is {course.minimum_age}.Please find another course')
            return redirect(reverse('courses'))
        elif course.maximum_age:
            if int(form_data.get('age')) > course.maximum_age:
                messages.error(request, f'Sorry, the maximum age for this\
                     course is {course.maximum_age}.\
                         Please find another course')
                return redirect(reverse('courses'))
        # Level Verification for B1+ classes
        lower_levels = ['A1', 'A2', 'A2H']
        course_level = form_data.get('course_level')
        if course.course_levels == "b1" and course_level in lower_levels:
            messages.error(request, f'Sorry, the minimum level for this\
                 course is {course.course_levels}.Please find another course')
            return redirect(reverse('courses'))
        # Level Verification for C1 only classes
        if course.course_levels == "c1" and course_level != 'C1':
            messages.error(request, f'Sorry, the minimum level for this \
                course is {course.course_levels}.Please find another course')
            return redirect(reverse('courses'))
        else:
...
def checkout(request, booking):
    ...
    end_date = course_start_date + datetime.timedelta(
                            weeks=int(booking.course_length))
    if request.method == 'POST':
